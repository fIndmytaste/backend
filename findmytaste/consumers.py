...
class OrderTrackingConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        import logging
        logging.warning(f"[OrderTrackingConsumer] Disconnect called for {self.room_group_name} ({self.channel_name})")
        await self.channel_layer.group_discard(
            self.room_group_name,
            self.channel_name
        )
        logging.warning(f"[OrderTrackingConsumer] Disconnect finished for {self.room_group_name} ({self.channel_name})")
...
class OrderTrackingConsumerOld(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        logging.warning(f"[OrderTrackingConsumerOld] Disconnect called for {self.room_group_name} ({self.channel_name})")
        await self.channel_layer.group_discard(
            self.room_group_name,
            self.channel_name
        )
        logging.warning(f"[OrderTrackingConsumerOld] Disconnect finished for {self.room_group_name} ({self.channel_name})")
    ...
    async def tracking_update(self, event):
        tracking_data = event['tracking_data']
        logging.info(f"[OrderTrackingConsumerOld] Sending tracking_update: {tracking_data}")
...
class DeliveryTrackingConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        import logging
        logging.warning(f"[DeliveryTrackingConsumer] Disconnect called for {self.room_group_name} ({self.channel_name})")
        await self.channel_layer.group_discard(
            self.room_group_name,
            self.channel_name
        )
        logging.warning(f"[DeliveryTrackingConsumer] Disconnect finished for {self.room_group_name} ({self.channel_name})")
    ...
    async def delivery_update(self, event):
        ...
        # add log
        logging.info(f"[DeliveryTrackingConsumer] Sent {event['type']} update: {event['data']}")
    ...
    async def rider_location_update(self, event):
        ...
        # add log
        logging.info(f"[DeliveryTrackingConsumer] Sent rider_location update: {event['data']}")
    ...
    async def order_status_update(self, event):
        ...
        # add log
        logging.info(f"[DeliveryTrackingConsumer] Sent status_update: {event['data']}")
...
class TestConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        import logging
        logging.warning(f"[TestConsumer] Disconnect called for {self.room_group_name} ({self.channel_name})")
        await self.channel_layer.group_discard(
            self.room_group_name,
            self.channel_name
        )
        logging.warning(f"[TestConsumer] Disconnect finished for {self.room_group_name} ({self.channel_name})")
...
class VendorNotificationConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        import logging
        logging.warning(f"[VendorNotificationConsumer] Disconnect called for {getattr(self, 'vendor_group_name', None)} ({self.channel_name})")
        if hasattr(self, "vendor_group_name"):
            await self.channel_layer.group_discard(
                self.vendor_group_name,
                self.channel_name
            )
        logging.warning(f"[VendorNotificationConsumer] Disconnect finished for {getattr(self, 'vendor_group_name', None)} ({self.channel_name})")
    ...
    async def new_order_notification(self, event):
        ...
        # add log
        logging.info(f"[VendorNotificationConsumer] Sent new_order notification: {event['data']}    ")
    ...
    async def order_accepted_notification(self, event):
        ...
        # add log
        logging.info(f"[VendorNotificationConsumer] Sent order_accepted notification: {event['data']}")
...
class CustomerNotificationConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        import logging
        logging.warning(f"[CustomerNotificationConsumer] Disconnect called for {getattr(self, 'customer_group_name', None)} ({self.channel_name})")
    ...
    async def order_accepted_notification(self, event):
        ...
        # add log
        logging.info(f"[CustomerNotificationConsumer] Sent order_accepted notification: {event['data']}")
...
#     async def connect(self):
#         # Add the rider to the riders_group
#         self.group_name = "riders_group"
#         await self.channel_layer.group_add(
#             self.group_name,
#             self.channel_name
#         )
...
#     async def disconnect(self, close_code):
#         # Remove the rider from the riders_group
#         await self.channel_layer.group_discard(
#             self.group_name,
#             self.channel_name
#         )
...
#         # Send the new order event to the rider
...
#     async def order_accepted_notification(self, event):
#         # Send the order accepted notification to the WebSocket
#         await self.send_json(event)
#         # add log
