...
class EmailService:
    ...
    def send_welcome_email_to_buyer(self, user_email, user_name):
    # def send_verification_code(self, user_email, user_name, verification_code):
        """Send account activation code to a user"""
        return self.send_email_with_template(
            email=user_email,
            template_name="emails/customer/customer_signup_welcome.html",
            template_data={
                "user_name": user_name,
                "current_year": "24 hours", 
                "app_name": settings.APP_NAME if hasattr(settings, 'APP_NAME') else "Findmytaste",
            },
    ...
    def send_verification_code(self, user_email, user_name, verification_code):
    # def send_welcome_email_to_buyer(self, user_email, user_name, verification_code):
        return self.send_email_with_template(
            email=user_email,
