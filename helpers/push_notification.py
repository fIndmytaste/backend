    ...
    def send_to_user_async(
        ...
        def _send_notification():
            try:
                ...
                # print(f"_send_notification: Resolved user {user} to {user_obj}")
                if not user_obj:
                    logger.error(f"User not found: {user}")
                    return {"success": False, "error": "User not found"}
                
                print(f"_send_notification: Sending notification to user {user_obj} (ID: {user_obj.id})")
                # Send notification
...
def send_order_payment_success_notification(
    ...
    Args:
        user: User object, user ID, or username
        order_id: Order ID for the successful payment
        amount: Optional payment amount to include in notification
        callback: Optional callback function to execute after sending
        
    Returns:
        Thread object for the asynchronous notification
    """
    print(
        f"send_order_payment_success_notification: user={user}, order_id={order_id}, amount={amount}"
    )
...
def send_login_notification(user: Union[User, int, str], callback: Optional[Callable] = None) -> threading.Thread:
    """
    Send login notification to user when they successfully log in
    ...
    try:
        ...
        user_obj = notification_helper._resolve_user(user)
        if not user_obj:
            logger.error(f"User not found for login notification: {user}")
            return None
        
        # Determine user type based on role
        user_type = "customer"  # default
        if hasattr(user_obj, 'role'):
            if user_obj.role in ['vendor']:
                user_type = "vendor"
            elif user_obj.role in ['rider']:
                user_type = "rider"
            elif user_obj.role in ['admin']:
                user_type = "customer"  # admin uses customer templates
        ...
        notification = notification_manager.create_notification(
            ...
            template_key="login",
            variables={"name": user_obj.first_name or user_obj.full_name}
        )
        
        if not notification:
            logger.error(f"Failed to create login notification template for user {user_obj.full_name}")
            return None
