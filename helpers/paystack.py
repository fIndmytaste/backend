...
class PaystackManager:
    ...
    def validate_bank(self,bank_code):
        success , banks_response = self.banks()
        if not success:
            return False , banks_response
        exist = list(filter(lambda x: x.get('code') == bank_code, banks_response))
        if not exist:
    ...
    def resolve_bank_account(self,account_number,bank_code):
        is_valid, bank_object = self.validate_bank(bank_code)
        if not is_valid:
            return False , bank_object
        url = f'{self.base_url}/bank/resolve?account_number={account_number}&bank_code={bank_code}'
        response = requests.get(url, headers=self.get_header())
        if response.ok:
            res = response.json()['data']
            res['bank_name'] = bank_object['name']
            return True , res
        return False , "Account could not be resolve"
    ...
    def initiate_payment(self, request, amount,order, is_mobile=False, promo_info=None):
        ...
        try:
            ...
            if response.ok:
                ...
                new_response['promo_info'] = promo_info
